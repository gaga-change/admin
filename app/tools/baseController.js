...
module.exports = (router, service, name) => {
    // 列表 - 接口
    ...
    // 列表(单) - 接口
    router.get(`/api/${name}/:objectId`, async ctx => {
    ...
    // 删除 - 接口
    router.delete(`/api/${name}/:objectId`, async ctx => {
        const objectId = ctx.params.objectId
        let object = {
            _id: objectId
        }
    ...
    // 删除多个 - 接口
    router.delete(`/api/${name}`, async ctx => {
        let ids = ctx.query.ids
        ids = ids.split(`,`)
        ctx.body = await service.delMore(ctx, ids)
    })
    // 修改 - 接口
    router.put(`/api/${name}/:objectId`, async ctx => {
        const {
            body
        } = ctx.request
        let object = body
