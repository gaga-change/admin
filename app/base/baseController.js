...
module.exports = (router, service, name, ...other) => {
    // 列表 - 接口
    router.get(`/api/${name}`, tools.checkAuth, ...other, async ctx => {
    ...
    // 列表(单) - 接口
    router.get(`/api/${name}/:objectId`, tools.checkAuth, ...other, async ctx => {
    ...
    // 增加 - 接口
    router.post(`/api/${name}`, tools.checkAuth, ...other, async ctx => {
        const {
            body
        } = ctx.request
        let object = body
    ...
    // 删除 - 接口
    router.delete(`/api/${name}/:objectId`, tools.checkAuth, ...other, async ctx => {
        const objectId = ctx.params.objectId
        let object = {
            _id: objectId
        }
    ...
    // 删除多个 - 接口
    router.delete(`/api/${name}`, tools.checkAuth, ...other, async ctx => {
        let ids = ctx.query.ids
        ids = ids.split(`,`)
        ctx.body = await service.delMore(ctx, ids)
    })
    // 修改 - 接口
    router.put(`/api/${name}/:objectId`, tools.checkAuth, ...other, async ctx => {
        const {
            body
        } = ctx.request
        let object = body
